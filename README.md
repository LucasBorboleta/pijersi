...
[Les règles en français dans la seconde partie du document](#Pijersi-:-les-règles-du-jeu)
...
You (either white or black player) has 14 cubes in your color. The 6 identical faces of a cube define its role (for example, rock). In each color, the cubes are divided into 2 qualities:
...
Knowing that a "unit" refers to a cube or a stack, only the "sky" units capture each other, and regardless of their heights (for example, a cube can capture a stack). "Earth" units never capture and are never captured.
...
La pile construite pendant votre tour peut être déplacée, et réciproquement, la pile déplacée pendant votre tour peut être dépilée en déplaçant son sommet. Cet enchaînement de 2 actions est possible une seule fois pendant votre tour.
